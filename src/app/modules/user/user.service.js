...
const createEmployee = async (employeeData) => {
    ...
    const hashedPassword = await bcrypt.hash(employeeData.password, Number(envVariables.BCRYPT_SALT_ROUND));
    const userData = {
...
const createManager = async (managerData) => {
    ...
    const hashedPassword = await bcrypt.hash(managerData.password, Number(envVariables.BCRYPT_SALT_ROUND));
    const userData = {
...
const updateMyProfile = async (user, updateData) => {
    const userInfo = await prisma.user.findUnique({
        where: {
            email: user?.email,
            status: ActiveStatus.ACTIVE,
