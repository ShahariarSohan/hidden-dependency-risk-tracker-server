...
const getAllSystem = async (params, options) => {
...
const softDeleteSystem = async (id) => {
    ...
    if (activeTask) {
        throw new AppError(httpStatus.BAD_REQUEST, "System has active tasks. Reassign or complete them first.");
    }
    // Option: prevent soft-delete if system is still assigned to a team
    if (system.teamId) {
        throw new AppError(httpStatus.BAD_REQUEST, "System is assigned to a team. Unassign or reassign the system before deletion.");
    }
    // Soft-delete
...
const updateSystemStatus = async (id, status) => {
    return prisma.system.update({
        where: { id },
        data: { status },
    });
};
const addSystemToTeam = async (systemId, teamId) => {
    ...
    if (system.teamId === teamId) {
        throw new AppError(httpStatus.BAD_REQUEST, "System is already assigned to this team");
    }
