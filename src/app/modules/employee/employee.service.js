...
const getAllEmployee = async (params, options) => {
...
const softDeleteEmployee = async (id) => {
    ...
    if (employee.teamId) {
        throw new AppError(httpStatus.BAD_REQUEST, "Employee is still assigned to a team. Remove from team before deleting.");
    }
    // ✅ Check active tasks
    ...
    if (hasActiveTasks) {
        throw new AppError(httpStatus.BAD_REQUEST, "Employee has unfinished tasks. Reassign or complete them first.");
    }
    // ✅ Atomic operation
...
const addEmployeeToTeam = async (employeeId, teamId) => {
    ...
    if (employee.teamId === teamId) {
        throw new AppError(httpStatus.BAD_REQUEST, "employee is already assigned to this team");
    }
